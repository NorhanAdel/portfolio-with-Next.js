...
export default function ExperienceSection() {
  ...
  return (
    <section id="experience" className="max-w-6xl mx-auto px-6 py-24 border-t border-white/5 relative z-10">
      <div className="mb-20 text-center">
        <span className="text-xs tracking-[0.3em] uppercase text-purple-400 block mb-3">
          {t.sub}
        </span>
      ...
      <div className="relative border-l-2 border-white/5 ml-4 md:ml-32 space-y-12 rtl:border-l-0 rtl:border-r-2 rtl:mr-4 rtl:md:mr-32 rtl:ml-0 rtl:md:ml-0">
        {experiences.map((exp, index) => (
          ...
          >
            <span className="absolute -left-[11px] rtl:-right-[11px] top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-[#060211] border-2 border-purple-400">
              <span className="h-1.5 w-1.5 rounded-full bg-purple-400" />
            </span>

            <div className="p-8 rounded-3xl border border-white/5 bg-white/[0.01] hover:bg-white/[0.03] backdrop-blur-md transition-all duration-300 hover:border-purple-500/30">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                  <h4 className="text-xl font-bold text-white flex items-center gap-2">
                    <FiBriefcase className="text-purple-400 w-5 h-5" />
                    {exp.role}
                  </h4>
              ...
              <ul className="space-y-3.5 mb-6">
                {exp.achievements.map((ach, achIdx) => (
                  <li key={achIdx} className="flex items-start gap-3">
                    <FiActivity className="text-purple-400/60 w-4 h-4 mt-1 flex-shrink-0" />
              ...
              <div className="border-t border-white/[0.03] pt-6 space-y-4">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-gray-500 mr-2 rtl:mr-0 rtl:ml-2">
                    {t.techUsed}
                  </span>
                  {exp.tags.map((tag, tagIdx) => (
                    <span 
                      key={tagIdx} 
                      className="text-[10px] font-semibold px-2.5 py-1 rounded-md bg-purple-500/10 border border-purple-500/10 text-purple-300"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <p className="text-gray-400 font-medium">
                    <strong className="text-white font-bold mr-1 rtl:mr-0 rtl:ml-1">
                      {t.outcome}
                    </strong>{" "}
                    {exp.outcome}
                  </p>
